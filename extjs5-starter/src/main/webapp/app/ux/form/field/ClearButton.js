 ...
 * 
 */
Ext.define('App.ux.form.field.ClearButton', {
...
createClearButtonEl : function() {
	var animateWithClass = this.animateClearButton
	...
	if (this.animateClearButton) {
		this.animateWithCss3 = this
				.supportsCssTransition(this.buttonEL);
	}
	if (this.animateWithCss3) {
		this.buttonEL.addCls(this.clearButtonCls + '-off');
	} else {
...
supportsCssTransition : function(el) {
	var styles = ['transitionProperty', 'WebkitTransitionProperty',
			'MozTransitionProperty', 'OTransitionProperty',
	...
	this.textField.inputEl.on('keydown', function(e) {
				if (e.getKey() == Ext.EventObject.ESC) {
					if (this.textField.isExpanded) {
						// Let combo box or date field first remove the
						// popup
						return;
					}
					// No idea why the defer is necessary, but otherwise
					// the call to setValue('') is ignored
					Ext.Function.defer(this.textField.setValue, 1,
							this.textField, ['']);
					e.stopEvent();
				}
			}, this);
...
addListeners : function() {
	...
	buttonEL
			.on('mouseover', this.handleMouseOverClearButton, this);
	buttonEL
			.on('mouseout', this.handleMouseOutOfClearButton, this);
	buttonEL.on('mousedown', this.handleMouseDownOnClearButton,
			this);
	buttonEL.on('mouseup', this.handleMouseUpOnClearButton, this);
	buttonEL.on('click', this.handleMouseClickOnClearButton, this);
},
...
handleMouseOverInputField : function(event, htmlElement, object) {
	this.buttonEL.addCls(this.clearButtonCls + '-mouse-over-input');
	if (event.getRelatedTarget() == this.buttonEL.dom) {
		...
		// will break)
		this.buttonEL.removeCls(this.clearButtonCls
				+ '-mouse-over-button');
		this.buttonEL.removeCls(this.clearButtonCls
				+ '-mouse-down');
	}
...
handleMouseOutOfInputField : function(event, htmlElement, object) {
	...
	if (event.getRelatedTarget() == this.buttonEL.dom) {
		...
		// Handle it here to avoid duplicate updates (else animation
		// will break)
		this.buttonEL.addCls(this.clearButtonCls
				+ '-mouse-over-button');
	}
	this.updateButtonVisibility();
},
...
handleMouseOutOfClearButton : function(event, htmlElement, object) {
	...
	this.buttonEL.removeCls(this.clearButtonCls
			+ '-mouse-over-button');
	this.buttonEL.removeCls(this.clearButtonCls + '-mouse-down');
	this.updateButtonVisibility();
},
...
handleMouseDownOnClearButton : function(event, htmlElement, object) {
	...
	this.buttonEL.addCls(this.clearButtonCls + '-mouse-down');
},
...
repositionClearButton : function() {
	...
	var clearButtonPosition = this
			.calculateClearButtonPosition(this.textField);
	buttonEL.dom.style.right = clearButtonPosition.right + 'px';
	buttonEL.dom.style.bottom = '0px';
},
...
isButtonCurrentlyVisible : function() {
	...
	var cachedVisible = Ext.core.Element.data(this.buttonEL.dom,
			'isVisible');
...
shouldButtonBeVisible : function() {
	...
	if (this.hideClearButtonWhenEmpty && Ext.isEmpty(this.textField.getValue())) {
		...
		var buttonEL = this.buttonEL;
		// noinspection RedundantIfStatementJS
		if (this.hideClearButtonWhenMouseOut && !buttonEL.hasCls(this.clearButtonCls+ '-mouse-over-button')&& !buttonEL.hasCls(this.clearButtonCls+ '-mouse-over-input')) {
			visible =  false;
		}else if(this.textField&&(this.textField.readOnly)){
			// add this property when you need to show clear me in readonly mode
			visible =  this.textField.showClearMe ? true : false;
		}
	}
	return visible;
},
...
updateButtonVisibility : function() {
	...
	if (oldVisible != newVisible) {
		if (this.animateClearButton && this.animateWithCss3) {
			this.buttonEL.removeCls(this.clearButtonCls
					+ (oldVisible ? '-on' : '-off'));
			buttonEL.addCls(this.clearButtonCls
					+ (newVisible ? '-on' : '-off'));
		} else {
			buttonEL.stopAnimation();
		...
		buttonEL.setStyle('background-color',
		...
		if (!(this.isTextArea && Ext.isGecko) && !Ext.isIE) {
			...
			var deltaPaddingRight = buttonEL.getWidth()
					- this.buttonEL.getMargin('l');
			var currentPaddingRight = this.textField.inputEl
					.getPadding('r');
			var factor = (newVisible ? +1 : -1);
			this.textField.inputEl.dom.style.paddingRight = (currentPaddingRight + factor
					* deltaPaddingRight)
					+ 'px';
			}
		}
