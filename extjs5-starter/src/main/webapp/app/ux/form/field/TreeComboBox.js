/**
 * A treecombobox control with support for autocomplete, remote loading, and many other features.
 *
 ...
 *     var states = Ext.create('Ext.data.TreeStore', {
 *      root: {
 *          expanded: true,
 *          children: [
 ...
 *     var cb = new App.ux.form.field.TreeComboBox(yourOptions);
 ...
 *             '<tpl for=".">',
 *                 '<div class="x-boundlist-item">{text} - {leaf}</div>',
 *             '</tpl>'
 *         ),
 ...
 *     });
 *
 * See also the {@link #treeConfig} option for additional configuration of the dropdown.
 *
 */
Ext.define('App.ux.form.field.TreeComboBox', {
    ...
    alternateClassName: 'App.ux.form.field.TreeCombo',
  ...
  //<locale>
    /**
     * @cfg {String} displayDelimiter
     * The character(s) used to separate the {@link #valueField value id} and {@link #displayField display text} of display item when
     * `{@link #renderIdWithText} = true`.
     */
    displayDelimiter: ' - ',
    //</locale>
     ...
     * `{@link #displayField}`.
     * 
     * if set `{@link #renderIdWithText}` to true, will take `{@link #valueField}`-`{@link #displayField}` as display text.
     * See also `{@link #renderIdWithText}`.
     *
     */
    valueField : 'id',
    
    /**
     * @cfg {Boolean} onlyLeafSelected
     * If set to `false`, allows the combo field to select Non-leaf node
     */
    onlyLeafSelected : false,
    
    /**
     * @cfg {Boolean} renderIdWithText
     * If set to `true`, the picker render valueField and displayField as display text
     */
     ...
     *
     *         var combo = new App.ux.form.field.TreeComboBox({
	 ...
	 *				            ] },
	 *				            { id: "3", text: "buy lottery tickets", leaf: true }
	 *				        ]
	 *				    }  // data is local
     *             }),
     *             valueField: 'id',
    ...
    /**
     * @cfg {Object} defaultTreeConfig
     * Set of options that will be used as defaults for the user-configured {@link #treeConfig} object.
     */
    defaultTreeConfig: {
    	rootVisible : false,
        useArrows : false,
     ...
     * An optional set of configuration properties that will be passed to the {@link Ext.tree.Panel}'s constructor.
     * Any configuration that is valid for TreePanel can be included. Some of the more useful ones are:
     *
     ...
     *         - field {@link App.ux.form.field.TreeComboBox ComboBox} This treecombobox
     *         - store {@link Ext.data.Store Store} This treecombobox's data store
     */
     ...
     * @param {App.ux.form.field.TreeComboBox} queryPlan.combo A reference to this ComboBox.
     ...
     * @param {App.ux.form.field.TreeComboBox} combo This combo box
...
//        }
        ...
        if (!displayTpl) {
            me.displayTpl = me.renderIdWithText ? new Ext.XTemplate(
                '<tpl for=".">' +
                	'{[typeof values === "string" ? values : values["' + me.valueField + '"]]}' + me.displayDelimiter +
                    '{[typeof values === "string" ? values : values["' + me.displayField + '"]]}' +
                    '<tpl if="xindex < xcount">' + me.delimiter + '</tpl>' +
                '</tpl>'
            ) : new Ext.XTemplate(
...
//        }
    ...
    onTypeAhead: function() {
        ...
        if (record) {
            newValue = me.renderIdWithText ? record.get(me.valueField) + me.displayDelimiter + record.get(displayField) : record.get(displayField);
            len = newValue.length;
            selStart = me.getRawValue().length;

//            tree.highlightItem(tree.getNode(record));
    ...
    onBindStore: function(store, initial) {
        ...
        if (store) {
            ...
            valueCollectionConfig = {
                ...
                // or the TreePanel operating, we must refresh our value.
                listeners: {
            ...
            // This is the selection model we configure into the dropdown TreePanel.
            // We use the selected Collection as our value collection and the basis
            // for rendering the tag tree.
            ...
            if (picker) {
                picker.selModel = me.pickerSelectionModel;
    ...
    doLocalQuery: function(queryPlan) {
        var me = this,
            anyMatchQueryString = '[\s\S]*',
            queryString = queryPlan.query,
            filters = me.getStore().getFilters(),
            filter = me.queryFilter,
            matchedNodeAndParentMap = new Ext.util.HashMap();
        ...
        if (queryString) {
            filter = me.queryFilter = new Ext.util.Filter({
            	filterFn: function(item) {
            		var match, displayValue, value, searchtest = me.caseSensitive ? 
            				( me.anyMatch ? new RegExp( anyMatchQueryString + queryString) : new RegExp(queryString) ) : 
            				( me.anyMatch ? new RegExp( anyMatchQueryString + queryString, 'i') : new RegExp(queryString, 'i'));
            		
            		displayValue = item.data[me.displayField];
            		value = item.data[me.valueField];
            		
            		match = displayValue ? searchtest.test(displayValue) : false;
            		
            		if( me.renderIdWithText ){
            			match = value ? match || searchtest.test(value) : match;
            		}
            		if(match){
            			me.putNodeAndParentToMap(item, matchedNodeAndParentMap);
            		} else {
            			match = matchedNodeAndParentMap.containsKey(value);
            		}
                    return match;
                }
            });
    ...
    // private
    putNodeAndParentToMap : function (node, map){
    	var me = this;
    	if(node){
    		map.add(node.data[me.valueField], node);
    		me.putNodeAndParentToMap(node.parentNode, map);
    	}
    },
...
//        	var lastSelected = me.picker.getSelectionModel().lastSelected, path = lastSelected ? lastSelected.getPath(): '';
//        	
//        	//TODO
//        	
////        	picker.seletedPath();
...
//                xclass: 'Ext.tree.Panel',
    ...
    onBeforeSelect: function(list, record, recordIndex) {
    	var me = this;
    	if(me.onlyLeafSelected){
    		return record.get('enabled')===false ? false : record.get('leaf') ? this.fireEvent('beforeselect', this, record, recordIndex) : false;
    	}
        return record.get('enabled')===false ? false : this.fireEvent('beforeselect', this, record, recordIndex);
    },
